...
match(this)-[r1:HAS_SUPPLIER_TERRITORY]-(ter:Territory)
With ter,this
match (this)-[r2:HAS_SUPPLIER_BUSINESSTYPE]-(b:BusinessType)
With ter,b,this
Match(b)--(t:Task)--(ter)
RETURN t
                                  """)
...
type Today {
  ...
  yearID:                         String @cypher(statement: """
                                  ...
                                  Return y.yearID
                                  """)
  month:                          [Day] @cypher(statement: """
                                  With date({timezone: 'America/New York'})as date
                                  Match(y:Year{year:date.year})--(m:Month{month:date.month})--(day:Day{day:date.day})--(w:Week)
                                  With m
                                  Return m
                                  """)
  week:                          [Day] @cypher(statement: """
                                  With date({timezone: 'America/New York'})as date
                                  Match(y:Year{year:date.year})--(m:Month{month:date.month})--(day:Day{day:date.day})--(w:Week)
                                  With w
                                  Return w
                                  """)
  year:                          [Day] @cypher(statement: """
                                  With date({timezone: 'America/New York'})as date
                                  Match(y:Year{year:date.year})--(m:Month{month:date.month})--(day:Day{day:date.day})--(w:Week)
                                  With y
                                  Return y
                                  """)
  date:                          [Day] @cypher(statement: """
                                  With date({timezone: 'America/New York'})as date
                                  Match(y:Year{year:date.year})--(m:Month{month:date.month})--(day:Day{day:date.day})--(w:Week)
                                  With date
                                  Return date
